...
package struct DescriptionPrescription: Identifiable {
    package var skill: Skill
    package var original: String
    package var rewritten: String
    /// 动了什么（人话逐条，含改不动的原因）
    package var moves: [String]
    /// 改写前后仍埋在 250 字符之外的「」触发词
    package var beforeBuried: [String]
    package var afterBuried: [String]
    /// 描述超过 1536 单条截断线（重排解决不了，要人删）
    package var overCap: Bool
    /// 机器无从下手（rewritten == original）
    package var noop: Bool { rewritten == original }
    package var id: String { skill.name }
}

package enum DescriptionRx {
    /// 生成处方。纯函数：不读盘不写盘。
    package static func prescribe(skill: Skill) -> DescriptionPrescription {
    ...
    package static func rewritePrompt(skill: Skill) -> String {
        """
    ...
    package static func writeBack(skill: Skill, newDescription: String) throws {
    ...
    package static func yamlSafe(_ value: String) -> String {
    ...
    package static func splitSentences(_ text: String) -> [String] {
    ...
    package static func buriedPhrases(in text: String) -> [String] {
    ...
    package static func visibleCount(in text: String) -> Int {
        guard let regex = try? NSRegularExpression(pattern: "「([^」]{1,24})」") else { return 0 }
        var count = 0
        var seen = Set<String>()
        let range = NSRange(text.startIndex..., in: text)
