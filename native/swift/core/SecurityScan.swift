...
package struct SecurityFinding: Identifiable, Hashable, Codable {
    package enum Severity: Int, Comparable, Codable {
        case critical = 0, warning = 1, info = 2
        package static func < (lhs: Severity, rhs: Severity) -> Bool { lhs.rawValue < rhs.rawValue }
    }
    ...
    package var id: String { "\(file):\(line):\(rule):\(excerpt.hashValue)" }

    /// 给第一次看到命中的人看：规则名换成一句能决定「先别用 / 打开看一眼」的话。
    package var beginnerNote: String {
...
package enum SecurityScanner {
    ...
    package static func scan(directory: URL) -> [SecurityFinding] {
        var cache = loadCache()
    ...
    package static func scanInstalled(targets: [(directory: String, path: String)]) -> [String: [SecurityFinding]] {
        var cache = loadCache()
    ...
    /// 目录里是否存在关键级发现（安装闸门用）
    package static func hasCritical(_ findings: [SecurityFinding]) -> Bool {
