...
package struct RepoRef: Equatable {
    package var owner: String
    package var repo: String
    package var branch: String?
    package var subpath: String?
    /// file:// 裸仓库时的直连克隆地址（离线验收用）
    package var cloneOverride: String?
    /// 本地文件夹安装（不是 git clone）
    package var localDirectory: String?

    package var cloneURL: String { cloneOverride ?? "https://github.com/\(owner)/\(repo).git" }
    package var display: String { "\(owner)/\(repo)" + (subpath.map { " → \($0)" } ?? "") }
    package var isLocalFolder: Bool { localDirectory != nil }
}

package struct InstallCandidate: Identifiable {
    ...
    /// 目标位置已有同名目录 → 只能跳过
    package var conflict: Bool
    package var selected: Bool
    /// 装前静态安全扫描结果（二期 F3）
    package var findings: [SecurityFinding] = []

    package var criticalCount: Int { findings.filter { $0.severity == .critical }.count }
    package var warningCount: Int { findings.filter { $0.severity == .warning }.count }
}

package struct InstallResult: Identifiable {
    package var id: String { directory }
    package var directory: String
    package var installed: Bool
    package var note: String
}

/// 安装管线对 App 状态的最小依赖。core 不能 import AppStore（那是 SwiftUI 层）。
@MainActor
package protocol InstallHost: AnyObject {
...
@MainActor
@Observable
package final class InstallerModel {
    ...
    // @Observable 自动追踪全部存储属性，不需要逐个标 @Published（那是 Combine 的机制，
    // core 禁依赖 Combine——见 PLAN.md §6 WP0：@Observable 允许留在 core）。
    package var urlText = ""
    package var stage = Stage.input
    ...
    package static func parse(_ input: String) -> RepoRef? {
    ...
    package static func adoptionRedirect(for path: String) -> String? {
    ...
    package func install(store: InstallHost) {
    ...
    /// 审阅页「仍要安装」：放行一次
    package func confirmReviewAndInstall(store: InstallHost) {
        reviewConfirmed = true
        install(store: store)
    }

    package func backToSelection() {
        stage = .selecting
    }
