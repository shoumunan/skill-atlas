...
/// Profile 里非成员技能的处置方式
enum ProfileExclusion: String, Codable, CaseIterable {
...
struct AtlasProfile: Codable, Identifiable, Equatable {
    /// UUID 字符串：改名不断绑定
    var id: String
    var name: String
    var symbol: String
    ...
    static func new(name: String, symbol: String = "square.grid.2x2") -> AtlasProfile {
        AtlasProfile(
...
struct ProfileBinding: Codable, Equatable, Identifiable {
...
struct ProfilesFile: Codable {
    var version: Int = 1
    var profiles: [AtlasProfile] = []
    /// 用户级默认 Profile（写 ~/.claude/settings.json），管所有没绑目录的会话
    var activeProfileID: String?
    var activeAppliedKeys: [String] = []
...
enum ProfileStore {
    static var url: URL { AtlasPaths.root.appendingPathComponent("profiles.json") }

    /// 读不到/解不开都返回空表。这里可以放心兜底：profiles.json 的「解码失败=空」
    /// 只会丢 Profile 定义，不像 atlas.json 那样会连带清空 145 条技能的挂载态。
    static func load() -> ProfilesFile {
    ...
    static func save(_ file: ProfilesFile) throws {
...
enum ProfileWriter {
    ...
    static func projectSettingsURL(for directory: URL) -> URL {
        directory.appendingPathComponent(".claude/settings.local.json")
    }

    static func plan(profile: AtlasProfile, skills: [Skill], target: URL) -> Plan {
    ...
    @discardableResult
    static func apply(profile: AtlasProfile, skills: [Skill], target: URL, previousKeys: [String]) throws -> [String] {
    ...
    static func revert(target: URL, appliedKeys: [String]) throws {
