...
## v16 · 2.3 四页结构与按原因归并（增量，与上文冲突以本章为准）
...
不设分组标题。需要分组本身就说明一级项太多。做技能不是天天干的事，「自己做一个」是「添加技能」里的一个入口，不占一格侧栏。

### 检查页：按原因归并，不按技能罗列

队列里一个技能一条会说谎。真机上 6 个技能同时报「GrokBuild 挂载缺失」——那不是 6 件事，是 1 件事的 6 个症状。页面必须按**原因**合并：

- 归并键是 `InboxItem.cause`，**不是 detail**。detail 里常带技能名（「打 /somd 就能用」），拿它当键会把同一个原因按名字打散。
- `cause` 为空 = 永不归并（审批这类必须逐条看原文）。
- 合并卡的标题是「N 个技能 + 原因」，技能名进副文案，最多列 3 个再写「等 N 个」。
- 有一次修完的动作才给批量键（挂载断了 → 全部重新挂上；被设成点名才用 → 全部改成自动）。**没有就不给**——点了不知道会发生什么的批量键比不给更糟。
- 侧栏角标也数**组**，不数条目。角标 14、页面 3 张卡是两个数字打架。

页面分两节，各有标题：「要你处理的」（待办，处理完就没了）和「顺便看一眼」（开场长度，是这个库长期的属性，不是待办）。账单在下面，不在上面。

### 全页只允许一个实心主按钮

六张卡各顶一个发光蓝按钮，等于六个「先点我」；一个都不给又等于没有入口。主按钮给「一键能修掉最多条」的那张，其余用 `quietControl(tint: accent)`。

### 侧栏：三项导航撑不满一根满高的栏

不拿别处已有的数字填空（技能总数在库页标题、待办数在角标、开场长度在检查页顶）。放**别处答不了的那个问题**：「Claude 最近到底在用哪几个」——`最近在用` 取 `usage.lastUsed` 前六，点一条跳到那个技能。没有使用记录时整块不出现，宁可空着也不摆空盒子。

底部给一句能验证的事实（技能存在这台电脑上，点开就是 Finder），不给孤零零的承诺。「原来的文件不动」没有上下文，谁的文件、什么时候动，用户一个都答不上来——那句承诺降为 tooltip 的补充说明。

### 暗色下的凸起：`Theme.raisedThumb`

分段控件里选中的那一格必须比所在面板**更亮**才读作凸起。不要用 `.background`：那是窗口背景色，暗色下（0.12）比面板（0.16）更深，选中项会读成一个凹下去的黑洞，和亮色下白色凸起的语义正好相反。同心圆角照旧：内圆角 = 外圆角 − 内缩。

### 平台标：十一个全有标，没有首字母灰块

一排图标里混两个字母块，用户看到的是「这两个没做完」。取标顺序：

1. 官方单色标（LobeHub，MIT）+ 品牌色上色 —— Qwen、Doubao、OpenCode
2. 官方标不可用时自绘原创 glyph —— OpenClaw（龙虾标有商标风险）、Hermes（官方标是插画人像，18pt 下糊成一团，也进不了这排几何标的体系）

彩色官方标带 `linearGradient` 的一律取单色版：`NSImage` 的 CoreSVG 渲染不出渐变 defs，渐变要在 SwiftUI 层做（Gemini 就是这么处理的）。

### 界面不许说旧黑话

`tests/assert_plain_language.py` 扫用户可见字符串（`L()`/`LF()`/`title:` 这类），命中「收件箱 / 供给页 / 发现页 / 创作页」即失败。注释不扫——注释里写「原供给页」是有用的历史说明。i18n 闸只查翻译齐不齐，查不出这个。

### 单个开关不得触发全量重扫

挂/摘一条软链只改这一个技能在这一个平台上的挂载态：技能内容没变（安全结论没变）、用法没变（索引没变）、版本没变（不必联网）。走 `SkillScanner.refreshMounts` 就地更新 + `patchMounts` 对齐汇总计数，**不走 `rescan()`**。

实测：旧路径每次点击付 511 ms 全量扫描，后面还挂着 147 个技能的安全全扫、用法索引重建、联网查更新；新路径 catalog 解析 0.32 ms + 11 次 lstat 0.023 ms，合计不到 1 ms。只有写盘失败才回退到全量重扫——那时盘上是什么样才是未知的。

## v17 · 2.4 场景跨平台（增量）

### 三档是 Claude Code 的，摘软链是别人的

场景（Profile）以前只对 Claude Code 生效，因为只有它有 `skillOverrides` 这个「装着、但不进开场清单」的开关。别的软件只有装/不装两档。

2.4 用它们有的那一档：**摘软链**。场景里勾了哪些软件，非成员技能在那些软件下的软链就先摘下来，撤场景时原样挂回。UI 必须把这个差别说出来，不能让用户以为两边是一回事。

### catalog 的 enabled 位一个字节都不动

`enabled[platform]` 是**你的意图**（「我要这个技能在 Codex 上用」），场景是**临时覆盖**。两者混在一个字段里就必然出事：

- 应用场景时把 enabled 改成 false → 撤场景时分不清哪些是场景关的、哪些是你本来就关的，一撤就把你自己关掉的也打开了；
- 不改 enabled、只摘软链 → 扫描器看到「说好要挂、盘上没有」，检查页立刻冒出一堆假的「装了但用不了」。

所以另记一本账：`~/.skill-atlas/scenario-mounts.json` 写明「这几条软链是场景摘的」。扫描器读这本账，把这些位置当成**有意关着**而不是挂载缺失（`SkillScanner.scan` 与 `refreshMounts` 两条路径都要读，口径必须一致）。

存独立文件的理由同 `profiles.json`：老版本 App 的 `AtlasCatalog.save()` 会把不认识的键整段丢掉，而 `~/.skill-atlas` 是 git 同步仓库，混版本机器一次保存就把账本清空——那之后就再也挂不回来了。

### 只记我们真摘掉的

应用时若某条软链本来就不在（你自己关的），**不记账也不碰**；占位是真目录的（不是我们挂的软链）一律不动。撤场景时只挂回账上记着的那些，且挂回前再确认 catalog 仍然说该挂——你在场景生效期间明确关掉的，尊重你后来的决定。

手动点亮平台图标时把那条从账上划掉：手动动作优先于场景。

### 验收必须证明「不多不少」

会动用户文件系统的功能，验收要证明三件事，缺一不可：

1. 应用后非成员在选中平台上确实摘掉了，且 catalog 的意图位没被改；
2. 摘掉的位置不算挂载失败（否则检查页全是假待办）；
3. 撤销挂回来的不多不少——你自己关掉的不会被替你打开。

夹具里会有前序用例留下的、与场景无关的挂载缺失，断言要**收窄到本用例摘的那个平台**，一刀切断言 0 会误伤。反过来这也是个对照组：没入账的缺失照报不误。

### CLI ABI 仍然冻结

`atlas profile apply` 多摘了几条软链，这件事只写进人读的那行，`data` 的形状不动（ADR-12）。JSON 消费方拿到的键与 2.3 完全一致。

## v18 · 2.5 侧栏放场景，不放日志（增量，推翻 v16 的「最近在用」）

v16 用「最近在用」填侧栏的空白，是**填充物**：最近用过哪几个技能，用户自己清楚，在侧栏再看一遍没有任何决定可做。当时的取舍标准写的是「放别处答不了的问题」——标准本身错了，新鲜 ≠ 有用。

**侧栏放的是你要切换的东西。** macOS 侧栏的形状就是「一组具名的东西，选一个」（Mail 的信箱、Notes 的文件夹、Finder 的位置）。场景正好是这个形状，而且：

- 它是**全局状态**：切了之后所有 AI 看到的技能清单都不一样，这种东西本来就该常驻 chrome；
- 它以前埋在 设置 → 进阶 → 场景包 → 管理场景… 四层底下，你根本看不出现在生效的是哪一套；
- 2.4 刚把它做成跨平台，做完却继续藏着，是自相矛盾。

### 规则

- **「全部技能」永远在第一位**，代表「没压任何场景」。你得能一眼看到当前没有场景，也得能一键退回来。
- 生效的那条用实心圆点标记，不靠颜色单独承载（无障碍：`accessibilityAddTraits(.isSelected)`）。
- 应用场景要写 `~/.claude/settings.json`、还要摘别的软件下的软链 → 走既有确认流程，先把写盘计划原样展示。退回「全部技能」是纯撤销，不拦。
- 一个场景都没有时，显示「配一套…」而不是空白：侧栏在这里顺带教了这个功能存在。
- tooltip 要说清这一套**管什么**，不能只报名字——留几个技能、Claude 那边什么效果、别的软件那边什么效果。
